...
ssize_t mock_write(int fd, const void *buffer, size_t size)
{
    (void)fd;
...
ssize_t mock_read(int fd, void *buffer, size_t size)
{
    (void)fd;
...
static void reset_mock(void)
{
    ...
    wrong_reply = short_output = short_input = timeout_input = stale_input = false;
}
...
int main(void)
{
    ...
    reset_mock();
    if (write_profile(-1, 1, input) || remaining || writes != 2 || memcmp(contents, input, BLOCK))
        fail("FAIL: synchronized two-chunk write");
    reset_mock();
    if (read_profile(-1, 1, output) || reads != 2 || memcmp(output, input, BLOCK))
    ...
    puts("PASS: mocked transport, readiness checks, partial transfers, timeouts, stale data.");
    return 0;
}
